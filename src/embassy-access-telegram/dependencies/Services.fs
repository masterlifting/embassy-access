    ...
    static member create (serviceId: Graph.NodeId) =
        fun (deps: Embassies.Dependencies) ->
            ...
            | Some Embassies.RUS ->
                ...
                | [ "0"; "0"; "1" ] ->
                    {
                        printPayload = EA.Russian.Services.Domain.Midpass.Payload.create >> Result.map EA.Russian.Services.Domain.Midpass.Payload.print
                        processRequest =
                            fun request ->
                                deps.Russian
                ...
                | _ ->
                    {
                        printPayload = EA.Russian.Services.Domain.Kdmid.Payload.create >> Result.map EA.Russian.Services.Domain.Kdmid.Payload.print
                        processRequest =
                            fun request ->
                                deps.Russian
            ...
            | Some Embassies.ITA ->
                {
                        printPayload = EA.Italian.Services.Domain.Prenotami.Payload.create >> Result.map EA.Italian.Services.Domain.Prenotami.Payload.print
                        processRequest =
                            fun request ->
                                deps.Italian
